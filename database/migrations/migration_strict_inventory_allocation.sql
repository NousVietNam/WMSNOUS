...
-- Fix allocate_outbound to enforce stricter inventory_type matching
-- Retail Orders (inventory_type='PIECE') must ONLY allocate from Retail Boxes ('PIECE' or NULL)
-- Bulk Orders (inventory_type='BULK') must ONLY allocate from Bulk Boxes ('BULK')
...
BEGIN
    ...
    LOOP
        ...
        FOR v_inv IN
            ...
            WHERE ii.product_id = v_item.product_id
              ...
              -- CRITICAL FIX: Match Inventory Type
              AND (
    ...
    IF array_length(v_errors, 1) > 0 THEN
        -- Rollback logic implies exception raising
