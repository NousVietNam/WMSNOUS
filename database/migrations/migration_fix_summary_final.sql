...
-- 0. DROP Old Functions (Handle ambiguity)
DROP FUNCTION IF EXISTS get_inventory_summary(uuid, text, text, text, text, text, text, text, text);
DROP FUNCTION IF EXISTS get_inventory_bulk_summary(uuid, text, text, text, text, text, text, text, text);

-- 1. Recreate RETAIL Summary RPC (get_inventory_summary)
-- No logic change, just ensuring clean state
...
-- 2. Recreate BULK Summary RPC (get_inventory_bulk_summary)
-- Fix: Use bi.warehouse_id for filtering
...
BEGIN
    ...
    inv_stats AS (
        ...
        WHERE 
            ...
            bi.quantity > 0 AND
            (p_warehouse_id IS NULL OR bi.warehouse_id = p_warehouse_id) AND -- <== FIXED: Use bulk_inventory.warehouse_id
            (p_location_code IS NULL OR l.code = p_location_code) AND
            (p_box_code IS NULL OR p_box_code = '' OR b.code = p_box_code)
    ),
    -- Soft Stats (Demand)
    soft_stats AS (
        SELECT 
