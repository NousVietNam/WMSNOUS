...
-- Migration: Strict Inventory Separation Logic (CORRECTED)
-- Description:
-- 1. Strictly enforce Inventory Separation:
--    - Retail Orders (PIECE) -> ONLY allocate from Retail Boxes ('PIECE' or NULL)
--    - Bulk Orders (BULK) -> ONLY allocate from Bulk Boxes ('BULK')
-- 2. Apply Sorting Strategy for BULK:
--    - Priority: Floor 1 (level_order = 1) -> Box Code Descending (Newest/Highest ID)
    ...
    LOOP
        ...
        FOR v_inv IN
            ...
            WHERE ii.product_id = v_item.product_id
              ...
              -- STRICT INVENTORY TYPE MATCHING (NO CROSS-DOCKING)
              AND (
                  -- Case A: Bulk Order -> Must take from Bulk Boxes
                  (v_order.inventory_type = 'BULK' AND b.inventory_type = 'BULK')
                  OR
                  -- Case B: Retail Order -> Must take from Retail Boxes
                  -- (Assuming NULL inventory_type on Box means Retail/Legacy)
                  (v_order.inventory_type = 'PIECE' AND (b.inventory_type = 'PIECE' OR b.inventory_type IS NULL))
              )
            ORDER BY 
                -- PRIORITIZATION STRATEGY
                CASE 
                    WHEN v_order.inventory_type = 'BULK' THEN
                        -- Bulk Strategy: Priority to Floor 1, then Descending Code
