-- Unified Ship RPC to handle outbound_orders
-- Supports SALE, TRANSFER, and INTERNAL types.

-- Ensure the column exists
ALTER TABLE outbound_shipments ADD COLUMN IF NOT EXISTS outbound_order_id UUID REFERENCES outbound_orders(id);
...
BEGIN
    ...
    IF v_order.status != 'PACKED' AND v_order.status != 'COMPLETED' THEN 
        -- status 'COMPLETED' is sometimes used for Manual Jobs finished picking
    ...
    -- 6. Create Outbound Shipment Record
    ...
    -- 9. Complete any related picking jobs if not already
