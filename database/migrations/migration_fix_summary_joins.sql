...
-- 1. Fix RETAIL Summary RPC (get_inventory_summary) - Use LEFT JOINs
...
-- 2. Fix BULK Summary RPC (get_inventory_bulk_summary) - Use LEFT JOINs
...
BEGIN
    ...
    inv_stats AS (
        ...
        LEFT JOIN boxes b ON bi.box_id = b.id -- <== CHANGED TO LEFT JOIN
        ...
        WHERE 
            ...
            (p_box_code IS NULL OR p_box_code = '' OR b.code = p_box_code) -- Handle empty string safely
    ),
    -- Soft Stats (Demand)
    soft_stats AS (
        SELECT 
