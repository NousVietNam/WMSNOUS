...
BEGIN
    IF p_inventory_type = 'BULK' THEN
        ...
        -- Note: using bi.warehouse_id instead of l.warehouse_id
        SELECT json_build_array(json_build_object(
            'locations', (
                ...
                WHERE bi.quantity > 0
                AND (p_warehouse_id IS NULL OR bi.warehouse_id = p_warehouse_id)
    ...
    ELSE
        -- PIECE INVENTORY LOGIC
        -- Note: using ii.warehouse_id instead of l.warehouse_id
        SELECT json_build_array(json_build_object(
            'locations', (
                SELECT json_agg(DISTINCT l.code)
