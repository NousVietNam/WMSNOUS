-- Force Refresh 
DROP FUNCTION IF EXISTS approve_outbound(UUID);
DROP VIEW IF EXISTS view_product_availability_bulk CASCADE;

-- 1. Ensure View exists and is correct
...
BEGIN
    ...
    -- Normalize
    v_inv_type := UPPER(COALESCE(v_order.inventory_type, 'PIECE'));
    ...
    LOOP
        ...
        IF COALESCE(v_available, 0) < v_item.quantity THEN
             v_missing := v_missing || jsonb_build_object(
                ...
                'debug_type', v_inv_type
             );
        END IF;
    END LOOP;

    -- If missing items, return error details including type info
    IF jsonb_array_length(v_missing) > 0 THEN
        RETURN jsonb_build_object(
            'success', false, 
            'error', 'Không đủ tồn kho khả dụng (' || v_inv_type || ')', 
            'missing', v_missing
        );
    END IF;
