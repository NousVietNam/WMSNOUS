-- =====================================================
-- Migration: Fix Allocate Logic for Locked Boxes
-- Description: Allow allocating from LOCKED boxes if they are held by the current order.
--              Previously, allocation only checked for OPEN boxes, causing failure for
--              "Ship Box" flows where boxes are locked upon approval.
-- =====================================================
...
BEGIN
    ...
    LOOP
        ...
        -- FIND INVENTORY (Optimized for First In First Out or specific logic)
        FOR v_inv IN
            ...
            WHERE ii.product_id = v_item.product_id
              ...
              -- CORE FIX: Allow OPEN boxes OR LOCKED boxes belonging to THIS order
              AND (
                  b.status = 'OPEN' 
                  OR 
