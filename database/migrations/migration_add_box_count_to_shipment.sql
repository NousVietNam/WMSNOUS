-- Add box_count column to outbound_shipments
ALTER TABLE outbound_shipments ADD COLUMN IF NOT EXISTS box_count INT DEFAULT 0;

-- Update the ship_outbound_order RPC to calculate and store box_count
...
BEGIN
    ...
    -- 4. Calculate Box Count (NEW)
    -- We simply count the boxes currently linked to this order.
    -- Since we do this BEFORE updating status/inventory, the boxes are all there.
    ...
    -- 7. Create Outbound Shipment Record (Updated with box_count)
    INSERT INTO outbound_shipments (
        ...
        picking_job_id,
        customer_name,
        box_count, -- Store the count permanently
        metadata
    )
    VALUES (
        v_pxk_code, 
