-- Migration: Fix Ship RPCs and missing columns
-- Description: Ensures outbound_order_id exists on outbound_shipments and redefines ship functions to be safe.

-- 1. Ensure columns exist
DO $$ 
BEGIN
    -- Ensure outbound_shipments has outbound_order_id
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'outbound_shipments' AND column_name = 'outbound_order_id') THEN
        ALTER TABLE outbound_shipments ADD COLUMN outbound_order_id UUID REFERENCES outbound_orders(id);
    END IF;

    -- Ensure outbound_shipments has picking_job_id (used by manual ship)
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'outbound_shipments' AND column_name = 'picking_job_id') THEN
        ALTER TABLE outbound_shipments ADD COLUMN picking_job_id UUID REFERENCES picking_jobs(id);
    END IF;
END $$;

-- 2. Redefine ship_outbound_order (For Orders)
...
BEGIN
    ...
    LOOP
        ...
        VALUES (
            ...
            'ITEM', -v_item.quantity, v_item.sku, v_shipment_id, v_item.box_id, v_user_id,
            'Xuất kho ' || v_pxk_code || ' (Đơn: ' || v_order.code || ')',
            NOW()
        );

        -- B. Delete Stock (Since it is shipped)
...
BEGIN
    ...
    -- Insert into outbound_shipments WITHOUT outbound_order_id (since Manual)
    INSERT INTO outbound_shipments (
