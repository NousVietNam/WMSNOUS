-- Migration: Safe Ship Outbound Order (v7)
-- Description: 
-- Adds explicit check for Job Type to prevent accidental shipping of Source Boxes in Item Picks.
...
BEGIN
    ...
    FROM (
        -- Method A: Outboxes (Explicitly assigned to order) -> Item Pick creates these
        SELECT id FROM boxes WHERE outbound_order_id = p_order_id
        
        UNION
        
        -- Method B: Source Boxes from BOX_PICK Jobs ONLY
        SELECT t.box_id AS id
        FROM picking_tasks t
        JOIN picking_jobs j ON t.job_id = j.id
        WHERE j.outbound_order_id = p_order_id
          AND t.status = 'COMPLETED'
          AND j.type = 'BOX_PICK' -- CRITICAL SAFETY CHECK: Only 'BOX_PICK' jobs ship the source box
    ) AS combined_boxes;
