...
export default function DoPickingPage() {
    ...
    const handleConfirmPick = async (task: PickingTask) => {
        // 1. Validation
        if (!activeOutbox) {
            alert("Vui lòng quét thùng Outbox trước!")
            setScannerMode('OUTBOX')
            setShowScanner(true)
            return
        }

        // 2. Synchronous Lock (Hard Lock)
        // Prevent double execution even if React hasn't re-rendered yet
        if (submittingRef.current.has(task.id) || task.status === 'COMPLETED') return
        submittingRef.current.add(task.id)
        setProcessingTasks(prev => new Set(prev).add(task.id))


        // 3. Optimistic UI Update (Immediate)
        // Update active outbox count immediately
        const previousOutbox = activeOutbox ? { ...activeOutbox } : null
        if (activeOutbox) {
            setActiveOutbox({ ...activeOutbox, count: (Number(activeOutbox.count) || 0) + task.quantity })
        }

        // Update task status immediately (This makes it Gray/Done)
        const previousTasks = [...allTasks]
        setAllTasks(prev => prev.map(t => t.id === task.id ? { ...t, status: 'COMPLETED' } : t))

        try {
            const { error } = await fetch('/api/picking/confirm', {
                method: 'POST',
                body: JSON.stringify({ taskId: task.id, outboxId: activeOutbox.id })
            }).then(r => r.json()).then(d => d.success ? { error: null } : { error: d.error })
            ...
            setActiveOutbox(previousOutbox)
            submittingRef.current.delete(task.id) // Release lock on error
        } finally {
            ...
            <div className="space-y-4 animate-in fade-in slide-in-from-right-4">
                ...
                <div className={`space-y-3 transition-opacity ${!isUnlocked ? 'opacity-50 pointer-events-none grayscale' : ''}`}>
                    {activeGroup.tasks.map(task => {
                        ...
                        return (
                            <div key={task.id} className={`rounded-xl border p-4 flex items-center justify-between gap-3 shadow-sm ${isDone ? 'bg-slate-50 border-slate-200' : 'bg-white border-blue-200 ring-1 ring-blue-100'}`}>
                                <div className="flex-1">
                                ...
                                <div className="text-right min-w-[80px]">
                                    <div className="flex flex-col items-end">
                                        <span className={`text-3xl font-black ${isDone ? 'text-slate-400' : 'text-blue-600'}`}>
                                            {task.quantity}
                                        </span>
                                ...
                                <button
                                    disabled={!isUnlocked || isProcessing || isDone}
                                    className={`h-14 w-14 rounded-full shadow-lg transition-all flex items-center justify-center
                                            ${isDone
                                            ? 'bg-slate-300 text-slate-700 shadow-none ring-0 cursor-not-allowed'
                                            : isProcessing
                                ...
                                >
                                    {isProcessing ? (
                                        ...
                                        <span className="text-sm font-bold">{picked}/{total}</span>
                                    ) : (
                ...
                {isCompleted && (
                    <button className="w-full mt-6 h-12 bg-slate-200 text-slate-800 font-bold rounded hover:bg-slate-300" onClick={() => setActiveBoxId(null)}>
                        Đóng Hộp
                    </button>
                )}
            </div>
